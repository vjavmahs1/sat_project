...
module.exports = function(passport){
  ...
  app.post('/set/new', l_set_upload.single('l_userfile_audio'), function(req, res, next){
    ...
    var sql = 'INSERT into `l_questionset` (`l_questionset_name`, `l_questionset_audiopath`) VALUES(?,?)';
    conn.query(sql,[l_questionset_name, l_questionset_audiopath], function(err, row){
      if(err){
        console.log(err);
       res.status(500).send('Internal Server Error');
     }else{
  ...
  app.post('/exam/new', function(req, res){
    ...
    var sql = 'insert into l_exam (l_exam_date) values(default)'
